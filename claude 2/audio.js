...
class CyberSFX {
    ...
    // Only the Cyberpunk theme gets this synth-glitch sound palette.
    isThemeActive() {
        return typeof document !== 'undefined' && document.body.getAttribute('data-theme') === 'cyberpunk';
    }
    ...
    // Short digital tick for piece movement / rotation.
    playMoveSound(isRotate = false) {
        if (!this.isThemeActive()) return;
        this.init();
        if (!this.ctx || this.volume <= 0) return;

        // Throttle so DAS auto-repeat doesn't turn into a wall of clicks
        const now = this.ctx.currentTime;
        if (now - this._lastMoveSoundTime < 0.03) return;
        this._lastMoveSoundTime = now;

        const gain = this.ctx.createGain();
        const vol = (isRotate ? 0.05 : 0.035) * this.volume;
        ...
        const osc = this.ctx.createOscillator();
        osc.type = 'square';
        osc.frequency.setValueAtTime(isRotate ? 1200 : 700, now);
        osc.frequency.exponentialRampToValueAtTime(isRotate ? 1900 : 500, now + 0.045);
        osc.connect(gain);
        osc.start(now);
        osc.stop(now + 0.05);
    }

    // Punchy low thud + zap for hard drop impact.
    playHardDropSound() {
        if (!this.isThemeActive()) return;
        this.init();
        if (!this.ctx || this.volume <= 0) return;
        ...
        thudGain.gain.setValueAtTime(0.22 * vol, now);
        ...
        zapGain.gain.setValueAtTime(0.1 * vol, now);
        zapGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.06);
        zap.connect(zapGain);
    ...
    // Two-tone swap sound for holding a piece.
    playHoldSound() {
        if (!this.isThemeActive()) return;
        this.init();
        if (!this.ctx || this.volume <= 0) return;
        ...
        const now = this.ctx.currentTime;
        const vol = this.volume;
        [ [1000, 0], [1600, 0.05] ].forEach(([freq, delay]) => {
            const osc = this.ctx.createOscillator();
            osc.type = 'triangle';
            osc.frequency.setValueAtTime(freq, now + delay);
            const gain = this.ctx.createGain();
            gain.gain.setValueAtTime(0.09 * vol, now + delay);
    ...
    playLevelUpSound() {
        ...
        const now = this.ctx.currentTime;
        const vol = this.volume;
        const notes = [523, 659, 784, 1046, 1318]; // C5 E5 G5 C6 E6 - triumphant rise
    ...
    // Descending glitch drone for game over.
    playGameOverSound() {
        if (!this.isThemeActive()) return;
        this.init();
        if (!this.ctx || this.volume <= 0) return;
        ...
        // Stutter glitch layer
        const glitch = this.ctx.createOscillator();
        glitch.type = 'square';
        [1200, 300, 900, 150, 600].forEach((f, i) => {
            glitch.frequency.setValueAtTime(f, now + i * 0.08);
        });
        const glitchGain = this.ctx.createGain();
        glitchGain.gain.setValueAtTime(0.05 * vol, now);
        glitchGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.45);
        glitch.connect(glitchGain);
        glitchGain.connect(this.ctx.destination);
        glitch.start(now);
        glitch.stop(now + 0.45);
    }

    // Crisp UI blip for menu button clicks.
    playMenuClickSound() {
        if (!this.isThemeActive()) return;
        this.init();
        if (!this.ctx || this.volume <= 0) return;
